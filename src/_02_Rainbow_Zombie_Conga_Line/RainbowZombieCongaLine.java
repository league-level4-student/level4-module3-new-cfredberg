...
public class RainbowZombieCongaLine {
	...
	public void jumpInTheLine(Zombie dancer, int position) {
		...
		for (int i = 0; i < congaLine.size(); i++) {
			if (i == position && i > 0 && i < congaLine.size()-1) {
	...
	public void rainbowBrains(Zombie dancer) {
		...
		for (ZombieHatColor color : ZombieHatColor.values()) {
			/*
			Node<Zombie> newZom = new Node<Zombie>(new Zombie(color));
			current.setNext(newZom);
			newZom.setPrev(current);
			current = current.getNext();
			*/
